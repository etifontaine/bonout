...
import { BoEvent, BoInvitationResponse, BoInvitationValidResponse } from "../../../../src/types";
import {
    createInvitationResponse, deleteInvitationResponse, getEventByLink,
...
export default async function handler(
    ...
    if (!event) {
        return res.status(400).json({ error: 'event was not found with this link' })
    }

    // Response was already added with for name
    const existingInvitation = event.invitations?.find(invitation => invitation.name === payload?.name)
