...
export default function InvitationModal({ link, userResponse }: IModal) {
            ...
            switch (userResponse) {
                case BoInvitationValidResponse.NO:
                    setContent({
                        ...
                        description: 'Veuillez confirmer que vous ne viendrez pas à cet événement'
                    })
                    break;
                case BoInvitationValidResponse.YES:
                    setContent({
                        title: 'Super! Vous pouvez venir',
                        description: 'Veuillez confirmer votre venue en indiquant votre nom ou pseudo'
                    })
                    break;
                case BoInvitationValidResponse.MAYBE:
                    setContent({
                        title: 'Vous n\'êtes pas sûr',
                        description: 'Veuillez confirmer indiquer votre nom ou pseudo pour confirmer votre choix'
                    })
                    break;
    ...
    return (
        <Transition.Root show={isOpen} as={Fragment}>
            <Dialog as="div" className="fixed z-40 inset-0 overflow-y-auto" initialFocus={cancelButtonRef} onClose={() => { }}>
                    ...
                    >
                        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div className="sm:flex sm:items-start">
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                        ...
                                        <form className="bg-white pt-6 pb-8" onSubmit={(e) => {
                                            e.preventDefault()
                            ...
                            <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                <button
                                    type="button"
                                    className={`w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 ${userResponse === BoInvitationValidResponse.NO ? 'bg-red-600 hover:bg-red-700 focus:ring-red-500' : 'bg-green-600 hover:bg-green-700 focus:ring-green-500'} text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm`}
